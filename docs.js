...
const codeValues=new Map(); let codeIndex=0;
const ic=n=>window.OrbitIcon?.(n)||'';
...
function command(slug,pm=manager){
 ...
 return {npm:`npm exec --yes --package=${pkg} -- orbit add ${slug}`,pnpm:`pnpm --package=${pkg} dlx orbit add ${slug}`,yarn:`yarn dlx -p ${pkg} orbit add ${slug}`,bun:`bunx --package ${pkg} orbit add ${slug}`}[pm];
}
...
function syncNavigation(){const open=document.body.classList.contains('nav-open');$('.docs-sidebar').inert=innerWidth<=700&&!open;$('#docs-menu').setAttribute('aria-controls','docs-navigation');$('.skip').href=location.pathname+'#docs-main';}
function iconize(){document.querySelectorAll('[data-doc-icon]').forEach(el=>el.innerHTML=ic(el.dataset.docIcon))}
function setDocumentTheme(theme){setTheme(theme);try{localStorage.setItem('orbit-theme',theme)}catch{}const button=$('#docs-theme');button.innerHTML=ic(theme==='dark'?'sun':'moon');button.setAttribute('aria-label',`Switch to ${theme==='dark'?'light':'dark'} theme`)}
function renderNavigation(){
 const term=$('#docs-search').value.trim().toLowerCase();
 const route=new URL(location.href).pathname;
 const link=(url,title,icon='',badge='')=>`<a class="docs-nav-link" href="${url}" ${new URL(url).pathname===route?'aria-current="page"':''}>${icon?ic(icon):''}${esc(title)}${badge?`<span class="docs-nav-badge">${badge}</span>`:''}</a>`;
 let html=`<div class="docs-nav-group"><h2>Workspace</h2>${link(base.href,'Overview','grid')}${link(new URL('gallery.html',base),'Component gallery','layers')}</div>`;
 if(!term)html+=`<div class="docs-nav-group"><h2>Guides</h2>${guideLinks.map(([id,name,icon])=>link(guideURL(id),name,icon,id==='migration'?'0.2':'')).join('')}</div>`;
 let found=0;
...
async function renderPreview(){
 ...
 if(mainTab==='usage'){panel.innerHTML=code(usage(current),'example.html','html');window.OrbitMotion.panel(panel);return;}
 if(mainTab==='code'){
  ...
  if(!text){try{const response=await fetch(new URL(filename==='app.css'?'styles.css':filename,base));if(!response.ok)throw Error('Source unavailable');text=await response.text()}catch{ text='Source could not be loaded. Download the source archive from Installation.';}}
  if(generation!==previewGeneration)return;
  $('#panel-source').innerHTML=code(text,filename==='app.css'?'styles.css':filename,sourceTab==='styles'?'css':'js');window.OrbitMotion.panel(panel);return;
 }
 ...
 instance=mounted;
 if(stateId!=='default')window.OrbitMotion.panel(instance.element);
}
...
async function renderRoute(){routePath=location.pathname;
 ...
 $('#docs-crumb').textContent='Overview';
 if(parts[0]==='components'){
  current=catalog.find(c=>c.slug===parts[1]);
  if(current){$('#docs-main').innerHTML=componentPage(current);$('#docs-crumb').textContent=current.name;document.title=`${current.name} — Orbit UI`;renderInstall();await renderPreview();}
  else $('#docs-main').innerHTML=`<h1>Component not found.</h1><p class="docs-description" style="margin-top:20px">That component isn’t in this version of Orbit.</p><a class="button" href="${base}" style="margin-top:24px">Back to the library ${ic('arrow')}</a>`;
 }else if(parts[0]==='guides'){
  const page=guideContent(parts[1],{code,command,apiTable,commonAPI});
  if(page){$('#docs-main').innerHTML=`<article class="docs-guide"><div class="docs-eyebrow">${ic('file')} Guides</div><h1>${page.title}</h1><p class="docs-description">${page.description}</p>${page.body}</article>`;$('#docs-crumb').textContent=guideLinks.find(g=>g[0]===parts[1])?.[1]||'Guide';document.title=`${$('#docs-crumb').textContent} — Orbit UI`;}
  else $('#docs-main').innerHTML='<h1>Guide not found.</h1>';
 }else{$('#docs-main').innerHTML=overview();document.title='Orbit — Component library';}
 if(generation!==routeGeneration)return;
 renderNavigation();iconize();syncNavigation();window.OrbitMotion.panel($('#docs-main'));
}
async function navigate(url){history.pushState(null,'',url);document.body.classList.remove('nav-open');$('#docs-menu').setAttribute('aria-expanded','false');await renderRoute();window.scrollTo({top:0,behavior:'instant'});$('#docs-main').focus({preventScroll:true});}
document.addEventListener('click',async e=>{
 const link=e.target.closest('a');
 if(link&&!e.metaKey&&!e.ctrlKey&&!e.shiftKey&&!e.altKey&&e.button===0&&!link.hasAttribute('download')){const url=new URL(link.href);if(url.origin===base.origin&&url.pathname.startsWith(base.pathname)&&!url.hash){const path=url.pathname.slice(base.pathname.length);if(!path||path==='index.html'||/^(components|guides)\/[^/]+\/?$/.test(path)){e.preventDefault();navigate(url);return;}}}
 ...
 if(b.id==='docs-menu'){const open=document.body.classList.toggle('nav-open');b.setAttribute('aria-expanded',open);syncNavigation();if(open)$('#docs-search').focus();}
});
document.addEventListener('keydown',e=>{
 if(e.key==='/'&&!e.target.closest('input,textarea,select')&&!document.querySelector('dialog[open]')){e.preventDefault();if(innerWidth<=700){document.body.classList.add('nav-open');$('#docs-menu').setAttribute('aria-expanded','true');syncNavigation()}$('#docs-search').focus()}
...
window.addEventListener('popstate',()=>{if(location.pathname!==routePath)renderRoute();});window.addEventListener('resize',syncNavigation);
window.addEventListener('orbit:navigate',e=>{const d=e.detail;if(d.component){const c=catalog.find(c=>c.name===d.component);if(c)navigate(componentURL(c));}else if(d.view==='screens')location.href=new URL(`gallery.html?view=screens&screen=${d.screen||'settings'}`,base);});
