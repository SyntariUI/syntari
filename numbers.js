...
(() => {
 ...
 function set(el,value,{kind=el.dataset.format||'number',initial=false}={}) {
  ...
  [...next].forEach((character,index)=>{
   ...
   SyntariMotion.animate(track,[{transform:`translateY(${direction>0?0:-distance}em)`},{transform:`translateY(${direction>0?-distance:0}em)`}],initial?800:650,{delay:initial?index*18:0});
  });
  el.dataset.previous=value;
  if(kindChanged)SyntariMotion.animate(el,[{opacity:0,translate:'0 5px'},{opacity:1,translate:'0 0'}],150);
 }
 ...
 function build(table,initial=true){
  ...
  tbody.querySelectorAll('tr').forEach((row,i)=>{const dataRow=data[i],people=dataRow[2]+step*(37-i*4),revenue=dataRow[3]?(dataRow[3]+step*(80-i*9)):0;
   ...
   if(initial)row.querySelectorAll('.rank-bars>span').forEach(bar=>SyntariMotion.animate(bar,[{transform:'scaleX(0)'},{transform:'scaleX(1)'}],650,{delay:i*35}));
 ...
 window.SyntariNumbers={prepare,reveal,set};
})();
