...
const sourceFiles = ['package.json', 'orbit.js', 'support.html', 'app.js', 'starter.js', 'agents.js', 'extras.js', 'controls.js', 'motion.js', 'numbers.js', 'docs-data.js', 'docs.js', 'docs.html', 'gallery.html', ...['tokens','styles','controls','motion','numbers','starter','agents','extras','docs'].map(name => `${name}.css`)];
...
const server = createServer(async (request, response) => {
  try {
    ...
    if (pathname === '/__inventory__') { response.writeHead(200, {'content-type':'text/html'}).end('<!doctype html><html data-theme="light"><head><meta charset="utf-8"><title>Orbit extraction</title></head><body><div id="inventory-stage" style="width:640px"></div></body></html>'); return; }
...
try {
  ...
  for (const component of components) {
    ...
    component.source = { registration:{file:registration.file,line:registration.text.slice(0,registration.text.indexOf(needle)).split('\n').length}, interactions:component.interactionSource, states:'docs-data.js', runtime:'orbit.js', docs:`https://giovanitier.github.io/orbit/components/${component.slug}/` };
