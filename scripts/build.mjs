...
const runtime=['orbit.js','support.html','tokens.css','styles.css','motion.css','numbers.css','controls.css','app.js','motion.js','numbers.js','controls.js','starter.js','starter.css','agents.js','agents.css','extras.js','extras.css'];
...
for(const c of components){
 ...
 const page=docs.replace('<head>','<head><base href="../../">').replace('<title>Orbit — Component library</title>',`<title>${c.name.replace(/&/g,'&amp;')} — Orbit UI</title>`);
...
for(const file of ['index.html','gallery.html','favicon.svg','docs.css','docs.js','docs-data.js','docs-guides.js',...runtime])await copyFile(file,'dist/'+file);
for(const dir of ['assets','components','guides','downloads'])await cp(dir,'dist/'+dir,{recursive:true});
console.log(`Built Orbit 0.2: ${components.length} component pages, 7 guides, gallery, and installable source archive.`);
