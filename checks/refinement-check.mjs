...
export default async page=>{
 ...
 await zone.locator('input').setInputFiles({name:'syntari-attachment.txt',mimeType:'text/plain',buffer:Buffer.from('Syntari test attachment')});check((await zone.innerText()).includes('syntari-attachment.txt'),'File selection displays local attachment');
 await zone.evaluate(el=>{const input=el.querySelector('input');input.click=()=>{el.dataset.pickerRequests=Number(el.dataset.pickerRequests||0)+1}});await zone.click();await zone.getByRole('button',{name:'Replace asset',exact:true}).focus();await page.keyboard.press('Enter');await page.keyboard.press('Space');check(await zone.getAttribute('data-picker-requests')==='3','Click, Enter and Space invoke picker');
 ...
 for(const theme of ['Light theme','Dark theme']){await setTheme(page,theme);await page.getByRole('button',{name:'Show toast',exact:true}).click();check(await page.locator('#toast').evaluate(el=>getComputedStyle(el).backgroundColor)==='rgb(20, 20, 31)',`${theme}: actual toast remains dark`);check(await page.locator('.toast-preview').evaluate(el=>getComputedStyle(el).backgroundColor)==='rgb(20, 20, 31)',`${theme}: toast specimen remains dark`)}
 await setTheme(page,'light');await page.locator('.docs-sidebar [data-view="gallery"]').click();return checks;
}
