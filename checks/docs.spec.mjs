...
test('documentation routes, examples, source, installation and navigation',async({page,context})=>{
 ...
 await page.goBack();await expect(page.locator('h1')).toHaveText('Agent todo list');await page.reload();await expect(page.locator('.orbit-component')).toBeVisible();expect(errors).toEqual([]);
});
test('all component pages and every authored preview state load without errors',async({page,request})=>{
 ...
 const errors=[];page.on('pageerror',e=>errors.push(e.message));await page.goto(origin+'/components/button/');await page.locator('.orbit-component').waitFor();
 const result=await page.evaluate(async()=>{
  const {getComponents,mount}=await import('/orbit.js');const {statesFor}=await import('/docs-data.js');const catalog=await getComponents();const errors=[];let states=0;
 ...
 console.log(result);expect(result.count).toBe(106);expect(result.states).toBeGreaterThan(212);expect(result.errors).toEqual([]);expect(errors).toEqual([]);
});
