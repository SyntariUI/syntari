...
test('visible state transitions: notifications, export, scheduler, upload and local file',async({page})=>{
 ...
 await open(page,'file-input');await live(page).locator('input[type=file]').setInputFiles({name:'sample.txt',mimeType:'text/plain',buffer:Buffer.from('Local preview')});await expect(live(page)).toContainText('sample.txt');await live(page).getByRole('button',{name:'Remove selected file'}).click();await expect(live(page)).toContainText('No assets selected');
...
test('pointer motion runs normally and reduced motion preserves final states',async({page})=>{
 await open(page,'tilt-card');const card=live(page).locator('.syntari-tilt');await card.hover({position:{x:20,y:20}});await expect.poll(()=>card.evaluate(el=>el.style.transform)).toContain('perspective');
...
test('mounted form names support review and confirmation and repeated save bar changes',async({page})=>{
 ...
 await open(page,'unsaved-changes');const input=live(page).getByRole('textbox');await input.fill('New name');await expect(live(page).getByRole('button',{name:'Save',exact:true})).toBeVisible();await input.fill('Syntari Studio');await input.fill('New name again');await expect(live(page).getByRole('button',{name:'Save',exact:true})).toBeEnabled();await live(page).getByRole('button',{name:'Save',exact:true}).click();await expect(live(page).locator('.unsaved-bar')).not.toBeVisible();await expect(input).toBeFocused();expect(errors).toEqual([]);
});
