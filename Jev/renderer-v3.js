...
const scenarios = {
  ...
  brief: {
    ...
    route: {
      ...
      copy: "The LLM summarizes project state after Logic fixes the UI shape.",
      footer: "Logic routes → LLM"
    },
...
function resetPlan() {
  $("[data-primary]").textContent = "Waiting for Logic…";
...
function resetRoute() {
  ...
  $("[data-route-copy]").textContent = "Logic decides whether open-ended reasoning is necessary.";
  ...
  if (foreignIds.length || preflightErrors.length) {
    ...
    preview.innerHTML =
      '<div class="generation-shell"><div class="generation-status">' +
        ...
        '<p>Syntari can only render components that pass the registry and component contracts.</p></div>' +
      '</div></div>';
    return;
  }
...
async function run(name, requestOverride = "") {
  ...
  $("[data-result-status]").textContent = "Logic is deciding…";
  ...
  setSkeletonStatus("Logic is deciding", "Typed decisions arrive independently instead of generating prose.");
...
function setInspector(mode) {
  ...
  if (!workspace) return;
  const current = workspace.dataset.panel || "none";
  const next = current === mode ? "none" : mode;
  workspace.dataset.panel = next;

  const open = next !== "none";
  $("[data-renderer-inspector]")?.setAttribute("aria-hidden", String(!open));
  if ($("[data-logic-panel]")) $("[data-logic-panel]").hidden = next !== "logic";
  if ($("[data-ir-panel]")) $("[data-ir-panel]").hidden = next !== "ir";
  $("[data-open-logic]")?.setAttribute("aria-pressed", String(next === "logic"));
  $("[data-open-ir]")?.setAttribute("aria-pressed", String(next === "ir"));
  if ($("[data-inspector-title]")) $("[data-inspector-title]").textContent = next === "ir" ? "Screen IR" : "Logic";
  if ($("[data-inspector-kicker]")) $("[data-inspector-kicker]").textContent = next === "ir" ? "Inspect structure" : "Inspect decisions";
}
...
function closeInspector() {
  ...
  $("[data-renderer-inspector]")?.setAttribute("aria-hidden", "true");
  $("[data-open-logic]")?.setAttribute("aria-pressed", "false");
  $("[data-open-ir]")?.setAttribute("aria-pressed", "false");
}

function toggleRendererFocus() {
  const shell = $("[data-renderer-shell]");
  if (!shell) return;
  const active = shell.classList.toggle("is-focus");
  $("[data-renderer-focus]")?.setAttribute("aria-pressed", String(active));
  if (active) closeInspector();
}

function setupRendererAmbientMotion() {
  const stage = $("[data-renderer-stage]");
  const cursor = $("[data-renderer-cursor]");
  if (!stage || !cursor || matchMedia("(pointer:coarse)").matches) return;
  ...
  stage.addEventListener("pointermove", event => {
    ...
    tx = rx;
    ty = ry;
    stage.style.setProperty("--mx", (rx / Math.max(rect.width, 1) * 100).toFixed(2) + "%");
    stage.style.setProperty("--my", (ry / Math.max(rect.height, 1) * 100).toFixed(2) + "%");
    cursor.classList.toggle("is-hot", Boolean(event.target.closest("button,a,input,textarea,select,[role=button]")));
  });
}

$("[data-open-logic]")?.addEventListener("click", () => setInspector("logic"));
$("[data-open-ir]")?.addEventListener("click", () => setInspector("ir"));
$("[data-close-inspector]")?.addEventListener("click", closeInspector);
$("[data-renderer-focus]")?.addEventListener("click", toggleRendererFocus);
window.addEventListener("keydown", event => {
  if (event.key === "Escape") {
    closeInspector();
    if ($("[data-renderer-shell]")?.classList.contains("is-focus")) toggleRendererFocus();
  }
});
setupRendererAmbientMotion();
