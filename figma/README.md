# Syntari → Figma: source extraction
...
The extractor starts a temporary server on a free localhost port, loads the actual Syntari runtime in Chromium, and closes both when finished. It uses the existing catalog and authored documentation recipes, not a separately maintained component list. It needs no Figma credentials or open Figma app. The local library is the input; no data is sent to Figma in this step.
